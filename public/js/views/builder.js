...
*/`),
      ...
      h('div', { class: 'kv' },
        ...
        h('span', { class: 'k' }, 'ai.ask(o)'), h('span', null, '{ prompt, system?, provider?, model?, maxTokens? } or just a prompt string -> Promise<text>, using Settings → AI (Anthropic, OpenRouter or Ollama). ctx.ai.available says whether AI is set up and allowed for sorts. The server makes the call; waiting on it does not count toward the time limit.'),
        h('span', { class: 'k' }, 'utils.claude(o)'), h('span', null, 'The 1.8 way: { apiKey?, prompt, model? } -> Promise<text>. Without an apiKey it uses Settings → AI.'),
